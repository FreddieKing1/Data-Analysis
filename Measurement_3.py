# -*- coding: utf-8 -*-
"""
Created on Thu Feb  3 11:30:22 2022
...
myvideo=cv2.VideoCapture('C:/Users/Freddie/Data/Measure_3.mp4')
numberframes=0
...
print(myvideo.read())
time=frame/16.73
plt.plot(time,displacement,color='red',label="Data")
plt.xlabel('Time (s)')
...
fit,cov=curve_fit(sin,time,displacement,p0=[250,7,0,500],sigma=weights)
fit_eq=sin(time,fit[0],fit[1],fit[2],fit[3])
plt.plot(time,fit_eq,label="Fit")         #This plots the best fit curve alonside the actual data
plt.legend()
plt.grid()
plt.show()
print(cov)
...
a_u=0.0058823529411
l_u=0.1
angle=np.arcsin((fit[0]*a)/l)
ang_u=(1/(np.sqrt(1-angle**2)))*np.sqrt(((a/l)*np.sqrt(cov[0][0]))**2+((fit[0]/l)*a_u)**2+(((fit[0]*a)/(l**2))*l_u)**2)
print("The angular displacement is",angle,"+-",ang_u)